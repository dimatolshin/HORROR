      ...
      {mediaQuery ? (
        <CustomSwiper isAbout config={{ slidesPerView: 1 }}>
          {CARD.map((element) => (
            ...
            >
              <span className="absolute text-[#9CBABC] z-10 top-[26%] left-[50%] translate-x-[-50%] translate-y-[-80%] text-[70px] font-[700] md:text-[102px]">
                {element.id}
              </span>
        ...
        <ul className="grid grid-cols-1 md:gap-y-[90px] lg:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-4 gap-5">
          {CARD.map((element) => (
            <li key={element.id} className="text-center relative">
              <span className="absolute text-[#9CBABC] z-10 top-[12%] left-[50%] translate-x-[-50%] translate-y-[-80%] text-[70px] font-[700] md:text-[102px]">
                {element.id}
              </span>
