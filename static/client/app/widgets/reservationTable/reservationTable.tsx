...
import type { ISlotsInfo } from "@/app/api/slots/fetchSlots";
...
export const ReservationTable = ({ quest }: ReservationProps) => {
    ...
    // ИСПРАВЛЕНИЕ: slot теперь опциональный, передаём info целиком
    ...
    return (
        <>
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    setSelectedSlot({
                                                        ...
                                                        info: item.info, // <-- Передаём весь массив info
                                                    });
                                                    setIsDialogOpen(true);
                                                }}
                                                className={`p-2 text-white rounded cursor-pointer md:text-[31px] ${
                                                    item.is_booked &&
            ...
            <ReservationModal
                ...
                questDetails={{
                    ...quest,
                    time: selectedSlot?.time ?? "00:00",
                    date_front: selectedSlot?.date_front ?? "",
                    info: selectedSlot?.info ?? [], // <-- Передаём info
                    // Убираем price и slot — они теперь динамические из info
                }}
